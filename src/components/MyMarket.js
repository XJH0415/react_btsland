...
export default class MyMarket extends React.Component{
    ...
    static defaultProps={
        pairs:DefaultConfig.defaultPairs
    }
    didBlur=null;
    didFocus=null;
    isShow=true;
    ...
    render(){
        return (
            <FlatList
                keyExtractor={item=>item.quote+'/'+item.base}
                data={this.props.pairs}
                ItemSeparatorComponent={()=><View style={{height:0.5,backgroundColor: '#D3D3D3'}}/>}
                renderItem={({item})=>{
                    var key=item.quote+'/'+item.base;
                    var timer=setInterval(()=>this.getMarket(item.base,item.quote),2000)
                    this.marketMap.set(key,timer);
                    return (<MarketItem ref={instance=>this.itemMap.set(key,instance)} base={item.base} quote={item.quote} />)
                }}
            />
        )
    }
